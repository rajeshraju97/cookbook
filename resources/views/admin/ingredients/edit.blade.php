...
@section('title', 'Ingredients Edit')
@section('content')

<div class="container">
    <div class="page-inner">
        <div class="page-header">
            <h3 class="fw-bold mb-3">Edit Ingredients</h3>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Edit Ingredients</h4>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.ingredients.update', $ingredient->id) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select form-control" name="dish_id">
                                    @foreach($dishes as $dish)
                                        <option value="{{$dish->id}}" {{ $ingredient->dish_id == $dish->id ? 'selected' : '' }}>
                                            {{$dish->dish_name}}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div id="ingredientsContainer" class="mb-3">
                                ...
                                @foreach(json_decode($ingredient->ingredients, true) as $name => $data)
                                    <div class="ingredient-row border rounded p-3 mb-3">
                                        <div class="row g-2">
                                            <div class="col-md-3">
                                                <label for="ingredientName{{$loop->index}}" class="form-label">Ingredient
                                                    Name</label>
                                                <input type="text" name="ingredients[{{$loop->index}}][name]"
                                                    id="ingredientName{{$loop->index}}" class="form-control"
                                                    value="{{ $name }}" required>
                                            </div>
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <input type="number" name="ingredients[{{$loop->index}}][price]"
                                                    id="price{{$loop->index}}" class="form-control"
                                                    value="{{ $data['price'] }}" min="0" required>
                                            </div>
                                            <div class="col-md-2">
                                                <label for="type{{$loop->index}}" class="form-label">Type</label>
                                                <select name="ingredients[{{$loop->index}}][type]" id="type{{$loop->index}}"
                                                    class="form-select" required>
                                                    <option value="variable" {{ $data['type'] == 'variable' ? 'selected' : '' }}>Variable</option>
                                                    <option value="fixed" {{ $data['type'] == 'fixed' ? 'selected' : '' }}>
                                                        Fixed</option>
                                                </select>
                                            </div>
                            ...
                            <!-- No of members -->
                            <div class="mb-3">
                                <label for="noOfMembers" class="form-label">No Of Members</label>
                                <input type="number" name="no_of_members" id="noOfMembers" class="form-control"
                                    value="{{ $ingredient->no_of_members }}" required>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        let ingredientCount = {{ count(json_decode($ingredient->ingredients, true)) }};
